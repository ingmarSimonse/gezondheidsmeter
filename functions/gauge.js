function createGauge(name, gaugeClass, value, gaugeMaxValue) {
    /*---------------------
    // D3.js Gauge Chart //
    ---------------------*/
    // Data which need to be fetched
    // var name = "Server Response";
    // var value = 10;			    // My Desired Value To Show
    // var gaugeMaxValue = 100;

    if (value == undefined) {
        value = $(`.${gaugeClass}`).data("value")
    }

    if (gaugeMaxValue == undefined) {
        gaugeMaxValue = $(`.${gaugeClass}`).data("maxvalue")
    }
    ...
    (function () {
        var barWidth, chart, chartInset, degToRad, repaintGauge, height, margin, numSections, padRad, percToDeg,
            percToRad, percent, radius, sectionIndx, svg, totalPercent, width;
